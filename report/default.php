 ...
 * 
 * This script defines default class for report plugins 
 */

////////////////////////////////////////////////////////////////////
///                                                            
/// The class doesn't do anything on it's own -- it needs to be extended.   
/// This class displays flax reports.  Because it is called from 
/// within /mod/flax/report.php you can assume that the page header
/// and footer are taken care of.
/// 
/// This file can refer to itself as report.php to pass variables 
/// to itself - all these will also be globally available.  You must 
/// pass "id=$cm->id" or q=$flax->id", and "mode=reportname".
////////////////////////////////////////////////////////////////////

// package mod/flax/report/; Included by mod/flax/report.php

class flax_default_report {
...
//        for ($i=0; $i<$count; $i++) {
//            $table->head[] = get_string('questionshort', 'flax', $i+1);
...
//    }
        ...
        if (isset($table->legend) && isset($value)) {
            ...
            if (empty($table->legend[$q])) {
                $table->legend[$q] = array(
                    'name' => flax_get_question_name($question),
                    'answers' => array()
                );
            }
    ...
    function create_legend_table(&$tables, &$table) {
        ...
        if (isset($table->legend)) {
            ...
            $legend->caption = get_string('reportlegend', 'flax');
            ...
            foreach($table->legend as $q=>$question) {
                ...
                $legend->stat[] = array(
                    get_string('questionshort', 'flax', $q+1), 
                    $question['name']
                );
...
/// print a report
    ...
    function print_report(&$course, &$flax, &$tables, &$options) {
        switch ($options['reportformat']) {
            case 'txt':
                $this->print_text_report($course, $flax, $tables, $options);
                break;
            case 'xls':
                $this->print_excel_report($course, $flax, $tables, $options);
                break;
    ...
    function print_report_start(&$course, &$flax, &$options, &$table) {
        switch ($options['reportformat']) {
            case 'txt':
                $this->print_text_start($course, $flax, $options);
                break;
            case 'xls':
                $this->print_excel_start($course, $flax, $options);
                break;

            case 'htm':
                $this->print_html_start($course, $flax, $options);
                break;
        }
    }
    ...
    function print_report_finish(&$course, &$flax, &$options) {
        switch ($options['reportformat']) {
            case 'txt' : 
                // do nothing
                break;
            case 'xls':
                $this->print_excel_finish($course, $flax, $options);
                break;
            case 'htm':
                $this->print_html_finish($course, $flax, $options);
                break;
        }
    }
...
//		print_simple_box_start("$table->tablealign", "100%", "#ffffff", 0);
...
/// print a text report
    ...
    function print_text_report(&$course, &$flax, &$tables, &$options) {
        $this->print_text_start($course, $flax, $options);
    ...
    function print_text_start(&$course, &$flax, &$options) {
        $downloadfilename = clean_filename("$course->shortname $flax->name.txt");
...
/// print an Excel report
    ...
    function print_excel_report(&$course, &$flax, &$tables, &$options) {
        global $CFG;
        ...
        // send HTTP headers
        $this->print_excel_headers($wb, $course, $flax);
    ...
    function print_excel_headers(&$wb, &$course, &$flax) {
        $downloadfilename = clean_filename("$course->shortname $flax->name.xls");
